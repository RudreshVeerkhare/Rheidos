...
def row_vector(row: Dict[str, str], columns: Sequence[str]) -> np.ndarray:
    return np.array([float(row[column]) for column in columns], dtype=float)


def component_unit_vector(position: np.ndarray, component: str) -> np.ndarray:
    ...
    if component == "r":
        return np.array(
            (position[0] / radial_length, 0.0, position[2] / radial_length),
            dtype=float,
        )
    if component == "theta":
        return np.array(
            (-position[2] / radial_length, 0.0, position[0] / radial_length),
            dtype=float,
        )
    raise ValueError(f"Unknown component {component!r}")


def project_component(position: np.ndarray, vector: np.ndarray, component: str) -> float:
    return float(np.dot(vector, component_unit_vector(position, component)))
...
def build_rows(
    ...
    for analytical_row, discrete_row in zip(analytical_rows, discrete_rows):
        ...
        position = 0.5 * (analytical_position + discrete_position)
        position_delta = float(np.linalg.norm(analytical_position - discrete_position))
...
def profile_y_limits(rows: Sequence[Dict[str, float | int | str]]) -> Tuple[float, float]:
    ...
    y_pad = 0.08 * max(float(np.ptp(y_values)), EPS)
    return float(np.min(y_values) - y_pad), float(np.max(y_values) + y_pad)
...
def plot_profile(
    ...
    radius = np.array([float(row["r"]) for row in plot_rows])
    u_exact = np.array([float(row["u_exact"]) for row in plot_rows])
    u_numerical = np.array([float(row["u_numerical"]) for row in plot_rows])
