...
class SopVerbRunner:
    ...
    def __init__(self, node_or_path: Any) -> None:
        ...
        self.node_path = (
            node_or_path if isinstance(node_or_path, str) else _node_path(self.node)
        )
        has_verb = getattr(self.node, "hasVerb", None)
        if callable(has_verb) and not bool(has_verb()):
            raise SopVerbUnavailableError(
                f"Node {self.node_path} does not expose a SOP verb"
            )
    ...
    def execute(
        ...
        *,
        ...
        hou = _get_hou()
        node = _resolve_node(self.node_path)
        ...
        if callable(min_inputs_fn):
            ...
            if len(input_list) < min_inputs:
                raise ValueError(
                    f"SOP verb for {self.node_path} requires at least "
                    f"{min_inputs} inputs, got {len(input_list)}"
                )

        if time is None:
            self.verb.loadParmsFromNode(node)
        else:
            self.verb.loadParmsFromNodeAtTime(node, float(time))
        if parms:
